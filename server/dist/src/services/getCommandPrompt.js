...
exports.getCommandPrompt = void 0;
...
const find_max_lemm_number = (all_data) => {
...
const getCommandPrompt = (question) => {
    ...
    responses.forEach((response) => {
        ...
        words.forEach((word) => {
            if (response["intents"].includes(word.lemma))
                input_true_lemm += 1;
        });
    ...
    if (expected_responses.length != 0) {
        let data = find_max_lemm_number(expected_responses);
        return data.response;
    }
    else
        return [];
};
exports.getCommandPrompt = getCommandPrompt;
//# sourceMappingURL=getCommandPrompt.js.map